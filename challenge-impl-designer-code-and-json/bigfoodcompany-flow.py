...
#----Pregunta agregada por consigna 2----
    preguntaConsentimientoDatos = Question(
        index="consentimientoDatos",
        text="Todas tus respuestas serán recolectadas y entregadas al equipo de Recursos Humanos de Sigma Alimentos. Estás de acuerdo?",
        errorMessageNotMatch="Creo que no te he entendido 🤔... Intenta con las opciones que te doy",
        exactMatch=True,
        options=["Sí!", "No estoy de acuerdo"],       
      ).get_dict()
#--------------------------------------
#Se modificó el "text" de esta pregunta por consigna 1---
...
#------------------------------------------------------
    ...
    #Pregunta modificada para cumplir consigna 3----
    preguntaNoInterest = Question(
        ...
        options = ["Empezar devuelta"],
        #immediateNext = True,     (entiendo esto ya no iría más por eso lo dejo comentado)
    ).get_dict()
    #--------------------------------
    ...
    # Se modifico conexión de esta pregunta por consigna 2----
    preguntaIntro["question"]["connections"].append({'goto':preguntaConsentimientoDatos["question"]["index"], 'isString': "Si"})
    #---------------------------------------------------------
    ...
    # Se modifico conexión de estas dos preguntas por consigna 2----
    preguntaQuienEsEmi["question"]["connections"].append({'goto':preguntaConsentimientoDatos["question"]["index"], 'isDefault': True})

    preguntaSosUnRobot["question"]["connections"].append({'goto':preguntaConsentimientoDatos["question"]["index"], 'isDefault': True})
    #---------------------------------------------------------
    # Se creó estas dos conexiones nuevas por consigna 2------
    preguntaConsentimientoDatos["question"]["connections"].append({'goto':preguntaInterest["question"]["index"], 'isString': "Sí!"})
    preguntaConsentimientoDatos["question"]["connections"].append({'goto':preguntaNoInterest["question"]["index"], 'isString': "No estoy de acuerdo"})
    #---------------------------------------------------------
    ...
    #Se agrega esta conexión para consigna 3-----
    preguntaNoInterest["question"]["connections"].append({'goto':preguntaIntro["question"]["index"], 'isString': "Empezar devuelta"})
    #--------------------------------------------
    ...
    #Se agrega esto por la nueva pregunta agregada por consigna 2, no se bien para que sirve supongo que es parte de la conexión--------
    flow[preguntaConsentimientoDatos["question"]["index"]] = preguntaConsentimientoDatos
    #-----------------------------------------------------
